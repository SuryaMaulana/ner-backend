...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://socialsift.biz.id/"],  
...
def process_csv(file_path):
    ...
    recap_data = {}
    for entity in df['Predicted_Entities'].dropna().unique():
        entity_data = df[df['Predicted_Entities'] == entity]

        total_count = len(entity_data)
        sentiment_count = entity_data['Sentiment'].value_counts().to_dict()
        label_count = entity_data['Predicted_Label'].value_counts().to_dict()

        recap_data[entity] = {
            "Total Count": total_count,
            "Sentiment": sentiment_count,
            "Classification": label_count
        }

    recap_file_path = os.path.join(GRAPH_FOLDER, 'recap_data.json')
    with open(recap_file_path, 'w') as f:
